...
export default function DropdownProfile({ align }: {
  ...
  return (
    <Popover className="relative inline-flex">
      ...
      <PopoverPanel 
        transition
        className={`origin-top-right z-10 absolute top-full left-0 right-auto min-w-[14rem] bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700/60 pt-1.5 rounded-xl shadow-lg overflow-hidden mt-1 focus:outline-none transition duration-200 ease-out data-[closed]:scale-95 data-[closed]:opacity-0 ${align === 'right' ? 'md:left-auto md:right-0' : 'md:left-0 md:right-auto'}`}
      >
        {({ close }) => (
          <>
